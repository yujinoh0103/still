...
public class Back_image_Ctrl : MonoBehaviour
{
    ...
    [Header("양 쪽 움직이는 버튼/ 왼[0] 오[1]")]
    public GameObject[] go_map_button_obj;
    [Header("거실 방 물품 클릭 오브젝트")]
    ...
    void Load_Image()
    {
        if ((player_now_floor_num == 1 && player_now_room_num ==9) || (player_now_floor_num == 1 && player_now_room_num == 11))
        {
           
            if (!livingroom_move)
            {
                go_map_button_obj[0].SetActive(false);
                go_map_button_obj[1].SetActive(true);

                switch (player_now_room_num)
                {
                    case 9:
                        gameObject.GetComponent<SpriteRenderer>().sprite = floor1_livingroom_image[0];
                        click_livingroom[0].SetActive(true);
                        click_livingroom[1].SetActive(false);
                        break;

                    case 11:
                        gameObject.GetComponent<SpriteRenderer>().sprite = floor1_livingroom_image[2];
                        click_livingroom[2].SetActive(true);
                        click_livingroom[3].SetActive(false);
                        break;
                }
            }
            else
            {
                go_map_button_obj[0].SetActive(true);
                go_map_button_obj[1].SetActive(false);

                switch (player_now_room_num)
                {
                    case 9:
                        gameObject.GetComponent<SpriteRenderer>().sprite = floor1_livingroom_image[1];
                        click_livingroom[0].SetActive(false);
                        click_livingroom[1].SetActive(true);
                        break;

                    case 11:
                        gameObject.GetComponent<SpriteRenderer>().sprite = floor1_livingroom_image[3];
                        click_livingroom[2].SetActive(false);
                        click_livingroom[3].SetActive(true);
                        break;
                }
            }
